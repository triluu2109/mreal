...
export default function DeleteButton({ id, action }: { id: string, action: (id: string) => Promise<{success: boolean, error?: string}> }) {
  const [isPending, startTransition] = useTransition();

  const handleDelete = () => {
    if (!confirm("Bạn có chắc chắn muốn xoá nhân sự này? Hành động này không thể hoàn tác.")) return;
